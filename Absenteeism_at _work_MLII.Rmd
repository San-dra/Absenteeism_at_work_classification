...
data.frame(Variable = names(abs_training_df),
           Type = sapply(abs_training_df, typeof),
           Min=sapply(abs_training_df, min),
           Max=sapply(abs_training_df, max),
...
The initial 5 box plot shows that We have a few outliers in Transport expense, service time, age, and hit.targe. Additionally there are outliers detected in work Load, height and number of pets, in the second group of boxplots. 
...
We have three outliers in number of pets owned. Eventhough it is not necessarly an error and employees can have as much as 5 to 8 pets we don't want our distribution to be affected by such values. At the time time we don't want to reduce the number of observations we have and thus the best option is to assign the median value to these three outliers. We will not consider having 4 pets as an outlier as it is not an extreme deviation. 


```{r Replacing Outlier - Number of Pets}
abs_dataset[abs_dataset$Pet > 4,1]
```
...
distance_bins<-bins.quantiles(abs_dataset$Distance.from.Residence.to.Work, 3, 3, verbose = FALSE)
distance_bins$binc
#[5, 24] [25, 41] [42, 52] we will use this range to create binned variable.
abs_dataset$Distance.Resi.to.Work_bin <-.bincode(abs_dataset$Distance.from.Residence.to.Work, c(5, 25, 42,2000), FALSE, TRUE)
abs_dataset$Distance.Resi.to.Work_bin <- recode(abs_dataset$Distance.Resi.to.Work_bin,`1`="Close",`2`="Far",`3`="Very Far")
